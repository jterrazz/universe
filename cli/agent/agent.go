...
universe. The Mind survives after the universe is destroyed.`,
}

func init() {
	Cmd.AddCommand(initAgentCmd)
	Cmd.AddCommand(listAgentCmd)
	Cmd.AddCommand(inspectAgentCmd)
	Cmd.AddCommand(spawnAgentCmd)
	Cmd.AddCommand(exportAgentCmd)
}
